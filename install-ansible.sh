...
readonly SHOULD_UPGRADE_ANSIBLE=false
readonly SHOULD_UPGRADE_ANSIBLE_LINT=false
...
if test $(python3 -m pip show ansible-base | grep -c 'ansible') = "0"; then
...
if test $(python3 -m pip show ansible-lint | grep -c 'Name: ansible-lint') = "0"; then
